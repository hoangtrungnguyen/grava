...
var showCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		query := `SELECT title, description, issue_type, priority, status, created_at, updated_at 
		...
		err := Store.QueryRow(query, id).Scan(&title, &desc, &iType, &priority, &status, &createdAt, &updatedAt)
		if err != nil {
