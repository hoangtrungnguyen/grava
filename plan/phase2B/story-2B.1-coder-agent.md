...
Implement a single grava task end-to-end via TDD. Each PR represents a single task (not an epic). Delegates everything to the `grava-dev-task` skill — including the claim. The skill's Step 2 (spec/scope check) runs before its Step 3 (claim), so a missing-spec HALT cannot leave a half-claimed issue.
...
- Fresh spawn: skill claims atomically (via `grava-dev-task` Step 3) → worktree at `.worktree/<id>` with branch `grava/<id>` → `CODER_DONE: <sha>` emitted
- Pre-claim HALT (no spec / scope mismatch): no claim recorded, no `grava stop` called, `CODER_HALTED: <reason>` emitted as last line
- Post-claim HALT: `grava stop <id>` runs (from skill), agent writes `coder_halted` wisp from repo root, `CODER_HALTED: <reason>` emitted as last line, worktree preserved
- RESUME spawn: skill's resume-detection branch skips re-claim, existing worktree used, continues work
- RESUME with `ROUND: N`: commit message contains `[round N]`
- RESUME with `FINDINGS_PATH`: agent reads file rather than inlining contents
- HALT preserves `.worktree/<id>` — agent does NOT clean up
- Final message's **last non-empty line** is exactly one signal string

## Dependencies

- `.claude/skills/grava-cli/` (exists)
- `.claude/skills/grava-dev-task/` (exists — task-scoped TDD workflow with internal claim handling)

## Signals Emitted

- `CODER_DONE: <sha>` — happy path
- `CODER_HALTED: <reason>` — blocked (pre- or post-claim)
