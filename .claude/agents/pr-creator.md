...
#### Style rules (MANDATORY — apply to every PR)

- **Single-line bullets only.** No multi-sentence bullets. No narrative paragraphs.
- **Concrete subjects.** File paths, test names, error codes, commit SHAs — not "we now improve resilience".
- **One header per logical block.** Use `## Problem`, `## Fix`, `## Test`, `## Files`. Add `## Residual` only when there's a known limitation. Drop sections that have nothing to say.
- **Two-line preamble** (`Issue:` + `Approved:`) before the first header. No "Generated by ..." footer.
- **Do NOT dump the issue description.** Summarize root cause in 1-3 short bullets under `## Problem`. The full description is already on the issue.
- **List files touched explicitly** under `## Files` (with `(new)` annotation when applicable). Reviewers should be able to see scope at a glance without `git diff --stat`.
- **Test section is concrete.** Name the test files and what they cover; don't write generic checkboxes like "code review pass".

#### Example (canonical — use this shape)
...
## Problem
- One-line statement of the bug or gap
- Symptom or trigger condition (when relevant)

## Fix
- Mechanism 1 (what changed, where)
- Mechanism 2
- Refactor note if any (e.g. testable seams added)

## Test
- `path/to/test.go`: N cases — list what they cover terse
- Sandbox / smoke results (when applicable)
...
# Compose BODY following the style rules above. Read the issue, the
# review findings, and the diff — DO NOT dump the issue description
# verbatim. Summarize. Keep bullets single-line.
#
# Required headers in order: ## Problem, ## Fix, ## Test, ## Files.
# Add ## Residual only when there's a documented limitation.
# Add the Epic line in the preamble only when EPIC_ID is non-empty.
BODY="$(your-body-here)"
```
